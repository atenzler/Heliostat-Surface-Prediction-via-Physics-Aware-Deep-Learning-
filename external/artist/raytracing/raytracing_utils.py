...
def reflect_all_multisun(
        incoming_ray_directions: torch.Tensor, reflection_surface_normals: torch.Tensor
) -> torch.Tensor:
    """
    Reflect incoming rays for multiple heliostats, each with its own incident ray direction.

    Parameters
    ----------
    incoming_ray_directions : torch.Tensor
        A tensor of shape (n, 4) representing the incident ray direction for each heliostat.
    reflection_surface_normals : torch.Tensor
        A tensor of shape (n, 10000, 4) where n is the number of heliostats and 10000 is the number
        of normal points per heliostat.

    Returns
    -------
    torch.Tensor
        A tensor of shape (n, 10000, 4) representing the reflected ray directions.
    """
    # Expand incoming ray direction from (n, 4) to (n, 1, 4) for broadcasting.
    incoming_ray_direction_expanded = incoming_ray_directions.unsqueeze(1)

    # Compute the dot product between the incident ray direction and each surface normal.
    # The result has shape (n, 10000, 1)
    dot_products = torch.sum(incoming_ray_direction_expanded * reflection_surface_normals, dim=-1, keepdim=True)

    # Compute the reflected ray directions using the reflection formula.
    reflections = incoming_ray_direction_expanded - 2 * dot_products * reflection_surface_normals
