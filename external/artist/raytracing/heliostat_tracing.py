...
def raytracing_debug(scenario, rays, image, heliostat_index):
    field = scenario.heliostat_field

    print(f"\n🔍 Heliostat {heliostat_index} DEBUG")

    # Surface origin
    origin_pts = field.all_current_aligned_surface_points[heliostat_index]
    z_vals = origin_pts[:, 2]
    print(f"  Surface Z (min/mean/max): {z_vals.min():.2f} / {z_vals.mean():.2f} / {z_vals.max():.2f}")

    # Ray directions
    ray_dirs = rays.ray_directions
    ray_z = ray_dirs[..., 2]
    print(f"  Ray Z dir mean: {ray_z.mean():.3f}, max len: {ray_dirs.norm(dim=-1).max():.3f}")

    # Flux result
    max_flux = image.max().item()
    print(f"  Max flux in image: {max_flux:.3f}")

    # Preferred reflection direction
    pref = field.all_preferred_reflection_directions[heliostat_index, 0]
    print(f"  Preferred reflection dir: [{pref[0]:.3f}, {pref[1]:.3f}, {pref[2]:.3f}]")

    if max_flux == 0:
        print("  ⚠️  No flux — check surface shape, aim, or ray direction.")
...
class HeliostatRayTracer:
    ...
    def __init__(
        ...
        # TODO: maybe this is not really optimal to just take the size of the first heliostat for everything
        self.number_of_surface_points_per_heliostat = (
            self.scenario.heliostat_field.all_current_aligned_surface_points.shape[1]
        )
        #self.distortions_loaders = list()
        #self.distortions_samplers = list()
        #self.distortions_datasets = list()

        #for heliostat in range(scenario.heliostat_field.number_of_heliostats):
        ...
        #self.distortions_datasets.append(distortions_dataset)
        ...
        #self.distortions_samplers.append(distortions_sampler)
    ...
    def trace_rays(
        ...
        #self.scenario.heliostat_field.all_preferred_reflection_directions = raytracing_utils.reflect_all_multisun(incoming_ray_directions=incident_ray_direction,
        ...
        self.scenario.heliostat_field.all_preferred_reflection_directions = raytracing_utils.reflect_all(incoming_ray_direction=incident_ray_direction,
                                                                                         reflection_surface_normals=self.scenario.heliostat_field.all_current_aligned_surface_normals)
        # Initialize the final bitmaps for all heliostats in the batch
        final_bitmaps = torch.zeros(
            (self.batch_size, self.bitmap_resolution_u, self.bitmap_resolution_e), device=device
        )

        for index, (batch_u, batch_e) in enumerate(self.distortions_loader):
            for batch_index in range(self.batch_size):
                ...
                # todo: this is done to make images, but correct?
                #mean_intersection = intersections.mean(dim=(0, 1, 2))
                #new_center = torch.cat([mean_intersection, torch.tensor([1.0], device=device)])

                # update a copy of the target area center — avoids breaking future calls
                #target_area_center = new_center  # temporary center just for this heliostat
                ...
                #print("Intersection Z (mean):", intersections[:, :, :, 2].mean().item())
                #print("Target center Z:", target_area.center[2].item())
                #print("Target size U/E:", target_area.plane_u, target_area.plane_e)

                final_bitmaps[batch_index] = total_bitmap

                #raytracing_debug(self.scenario, rays, total_bitmap, batch_index)
